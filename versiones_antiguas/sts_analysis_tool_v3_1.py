# sts_analysis_tool_v3_1.py
# -*- coding: utf-8 -*-
"""
Versión v3.1
- Detección de fases estilo Excel (ventana=30, n_positive=30, vel_th=±0.1 m/s por defecto)
- Marcadores 'Acc Phases' y tiempos de fase por marcadores [(r-1)*3+1 ..]
- Salida:
  * Hoja1_Variables (por muestra)
  * Hoja2_Parametros_Grafica (parámetros + imagen incrustada)
  * Hoja3_Kinematic_&_Forces_like (3 filas por repetición: Concéntrica, Excéntrica, Sentado)
- Métricas por fase (ampliadas en v3.1):
  * BCM_Z: rango desplazamiento (m), vel (m/s), potencia/trabajo (P≈m*g*v_z si --mass-kg)
  * HIP_Z: rango desplazamiento (m), vel (m/s) (nativo o derivado), potencia/trabajo si hay columnas nativas
  * EMG: medias por fase si existen columnas EMG (%):
      'RT TIB.ANT. (%)','RT VLO (%)','RT RECTUS FEM. (%)','RT MED. GASTRO (%)',
      'RT SEMITEND. (%)','RT GLUT. MAX. (%)','RT LUMBAR ES (%)'
  * CoP SD: si existen columnas 'CoP_SD Displ_AP','CoP_SD Displ_ML','CoP_SD Displ_Result', se promedia por fase;
            si existen columnas de desplazamiento crudo ('CoP_Disp_X','CoP_Disp_Y' o similares),
            se calcula SD por fase (STDEV.S) y se reporta como *_SD.

Compatibilidad: usa integración trapezoidal manual (no depende de np.trapz/np.trapezoid).
"""
...
class PhaseComputer:
    ...
    def power_work(self, power_col: Optional[str], vel_col: Optional[str], mass_kg: Optional[float], t0: float, t1: float):
        ...
        if power_col and (power_col in self.cols):
            ...
            # aproximación P = m*g*v_z si es BCM; para Hip_Z solo si existe columna de potencia nativa
            g = 9.80665
...
def run_v3_1(
    ...
    if time_col not in df.columns:
        raise ValueError('No se encontró la columna de tiempo')
    ...
    # hip z
    disp_hipz = next((c for c in df.columns if 'hip rt-z' in str(c).lower()), None)  # Noraxon MyoMotion-Trajectories-Hip RT-z (mm)
    vel_hipz_native = next((c for c in df.columns if 'veloc_hip_z' in str(c).lower() or 'vel hip z' in str(c).lower()), None)
    # potencia/trabajo hip nativos
    ...
    # CoP SD columnas confirmadas
    cop_sd_cols = {
        'CoP_SD Displ_AP': None,
        'CoP_SD Displ_ML': None,
        'CoP_SD Displ_Result': None,
    }
    # intentar localizar columnas crudas si hubiera que calcular SD
    # heurísticos habituales
    raw_cop_candidates = {
    ...
    # --- Detección de eventos estilo Excel ---
    n = len(z_mm)
    ...
    vup = (centered_slope(z_m, dt, half_window_derivative) >  vel_th_m_s)
    vdn = (centered_slope(z_m, dt, half_window_derivative) < -vel_th_m_s)
    ...
    # marcadores acumulados exactos (Acc Phases)
    acc = np.zeros(n, int)
    curr=0
    ...
    for ev in rep_events:
        for phase in ('Concéntrica','Excéntrica','Sentado'):
            ...
            row['BCM_Z_Max (m)']=mx; row['BCM_Z_Min (m)']=mn; row['BCM_Z_Range (m)']=rg
            mV, MV, mVn = pc.stats(None, vel_bcm_m_s, t0, t1)
            row['BCM_Z_Vel_Mean (m/s)']=mV; row['BCM_Z_Vel_Max (m/s)']=MV; row['BCM_Z_Vel_Min (m/s)']=mVn
            MP, XP, WK = pc.power_work(None, None, meta['Peso_kg'], t0, t1)  # P≈m*g*v_z (usa vel_bcm_z derivada)
            # Para usar vel_bcm_z derivada, recalculamos aquí
            sl = pc._slice(t0, t1)
            if meta['Peso_kg'] is not None:
                ...
                if np.any(np.isfinite(P_series)):
                    ...
                    WK = trapz_manual(P_series[np.isfinite(P_series)], dt)
            row['BCM_Z_Power_Mean (W)']=MP; row['BCM_Z_Power_Max (W)']=XP; row['BCM_Z_Work (J)']=WK

            # HIP Z
            if disp_hipz is not None:
                hmx,hmn,hrg = pc.range_mm_to_m(disp_hipz, t0, t1)
                row['HIP_Z_Max (m)']=hmx; row['HIP_Z_Min (m)']=hmn; row['HIP_Z_Range (m)']=hrg
            # velocidad hip: nativa o derivada si hay disp hip
            if vel_hipz_native and vel_hipz_native in df.columns:
                hVm, hVx, hVn = pc.stats(vel_hipz_native, None, t0, t1)
            elif disp_hipz is not None:
                # derivar en caliente a partir de columna hip z mm
                ...
                hVm=hVx=hVn=None
            row['HIP_Z_Vel_Mean (m/s)']=hVm; row['HIP_Z_Vel_Max (m/s)']=hVx; row['HIP_Z_Vel_Min (m/s)']=hVn

            # Potencia/Trabajo HIP: solo si hay columnas nativas; si no, se deja None (no aproximamos con masa total)
            if power_hipz_native and power_hipz_native in df.columns:
                # potencia media y máxima desde columna
                hPM = pc.mean(power_hipz_native, t0, t1)
                # máxima como max sobre el periodo
                hPX = pc.stats(power_hipz_native, None, t0, t1)[1]
                row['HIP_Z_Power_Mean (W)']=hPM; row['HIP_Z_Power_Max (W)']=hPX
            if work_hipz_native and work_hipz_native in df.columns:
                # si hay columna de trabajo instantáneo acumulado por muestra no tiene sentido; asumimos que la columna es trabajo acumulado por evento
                # si la columna es trabajo por muestra, integramos; si es acumulado, tomamos delta
                sl = pc._slice(t0, t1)
            ...
            # CoP SD
            # Si existen columnas de SD ya calculadas, promediamos por fase (como representación);
            # si existen crudas, calculamos SD (STDEV.S) sobre el tramo.
            # AP
    ...
    out_prefix = out_prefix or (os.path.splitext(file_path)[0] + '_analysis_v3_1')
    plot_path = out_prefix + '_segmentation.png'
    try:
        fig, ax1 = plt.subplots(figsize=(12,6)); ax2=ax1.twinx()
        ax1.plot(t, z_mm, 'b-', label='BCM Z (mm)')
        ax2.plot(t, centered_slope(z_m, dt, half_window_derivative), color='orange', alpha=0.7, label='Vel BCM Z (m/s)')
    ...
    if not hoja3.empty:
        for j,col in enumerate(hoja3.columns, start=1): ws3.cell(1,j,col).font=Font(bold=True)
        for i, (_, row) in enumerate(hoja3.iterrows(), start=2):
            for j,val in enumerate(row, start=1):
                if isinstance(val, float) and (np.isnan(val) or np.isinf(val)): val=None
                ws3.cell(i,j,val)
        for j in range(1, len(hoja3.columns)+1):
            ws3.column_dimensions[get_column_letter(j)].width = min(max(len(str(hoja3.columns[j-1]))+2,14),34)
    else:
...
if __name__ == '__main__':
    ap = argparse.ArgumentParser(description='STS v3.1: Acc Phases + Hoja3 ampliada (Hip Z, EMG, CoP SD)')
